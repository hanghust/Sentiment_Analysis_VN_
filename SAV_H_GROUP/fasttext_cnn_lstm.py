...
def buildWordVector(tokens, size):
    data = []
    for sent in tokens:
        vec = np.zeros([len(sent), size])
        count = 0
        for word in sent:
            try:
                vec[count] = model_fasttext[word]
                count += 1
...
train_vecs_fasttext = np.concatenate([buildWordVector(sentence_train, 150)])
print((train_vecs_fasttext[0][0]).shape)
val_vecs_fasttext = np.concatenate([buildWordVector(sentence_val, 150)])
# print(val_vecs_fasttext.shape)

test_vecs_fasttext = np.concatenate([buildWordVector(sentence_test, 150)])
# print(train_vecs_fasttext.shape[0])
# #########################################
